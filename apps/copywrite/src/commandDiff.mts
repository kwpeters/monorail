...
export function builder(argv: Argv): Argv {
    ...
    .example(
        `$0 diff -e **/node_modules/**/* -e **/.turbo/**/* -e **/dist/**/* \\path\\to\\left\\dir \\path\\to\\right\\dir`,
    ...
    .example(
        `$0 diff --include-left-only=false \\path\\to\\left\\dir \\path\\to\\right\\dir`,
        `Omit files that exist only in the left directory, showing only right-only and files present in both.`
    )
    .example(
        `$0 diff --include-right-only=false \\path\\to\\left\\dir \\path\\to\\right\\dir`,
