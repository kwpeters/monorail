...
export async function handler(args: Arguments): Promise<void> {
    ...
    const sourceDir: Directory = new Directory(args.sourceDir as PathPart);
    const sourceMapPromise = getFileMap(sourceDir);

    const destDir: Directory = new Directory(args.destDir as PathPart);
    const destMapPromise = getFileMap(destDir);

    await Promise.all([sourceMapPromise, destMapPromise])
    .then(([srcMap, dstMap]) => {
